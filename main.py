...
if __name__ == '__main__':
    ...
    while running:
        ...
        for line in lines:
            s_x, s_y, s_z = sum(vectors[:, 0] * line[:3]), sum(vectors[:, 1] * line[:3]), sum(vectors[:, 2] * line[:3])
            e_x, e_y, e_z = sum(vectors[:, 0] * line[3:]), sum(vectors[:, 1] * line[3:]), sum(vectors[:, 2] * line[3:])
            pygame.draw.line(screen, (255, 255, 255), (250 + s_x * scale * (1.0625 ** (s_y * 1.0625)), 200 + s_z * scale * (1.0625 ** (s_y * 1.0625))),
                             (250 + e_x * scale * (1.0625 ** (e_y * 1.0625)), 200 + e_z * scale * (1.0625 ** (e_y * 1.0625))), int(max((scale / 32, 1))))
        a1 += pi / 256 * what_we_see[26] - pi / 256 * what_we_see[22]
        a2 += pi / 256 * what_we_see[4] - pi / 256 * what_we_see[7]
        scale = scale / (1.0625 ** what_we_see[45]) * (1.0625 ** what_we_see[46])
        for v in vectors:
